...
unpack s with (strM s)
  unpack ""             | StrNil = []
  unpack (strCons x xs) | (StrCons x xs) = x :: assert_total (UnitaryOp.unpack xs)
...
applyU : {i:Nat} -> {n:Nat} -> Unitary i -> (v : Vect i Nat) -> Unitary n -> UState (Unitary n) (Unitary n) (Unitary n)
applyU {i} {n} ui v un = case (maybeIsInjective n v) of
                      (Just a)=> (do
...
applyUwithGet : {i:Nat} -> {n:Nat} -> Unitary i -> (v : Vect i Nat) -> UState (Unitary n) (Unitary n) (Unitary n)
applyUwithGet {i} {n} ui v = case (maybeIsInjective n v) of
                      (Just a)=> (do
                        un <- get
                        pure (Unitary.apply {i = i} {n = n} ui un v {prf = a}))
                      Nothing => MkUS (\u => do
