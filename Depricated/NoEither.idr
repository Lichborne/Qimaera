...
data Unitary : Nat -> Type where
  ...
  CNOT   : (c : Nat) -> (t : Nat) -> 
            {auto prf1 : LT c n} -> {auto prf2 : LT t n} -> {auto prf3 :  c = t -> Void } -> 
...
apply : {i : Nat} -> {n : Nat} -> 
        ...
        (v : Vect i Nat) -> 
        {auto prf : (IsInjectiveDec n v)} -> 
        Unitary n
apply IdGate g2 _ = g2
apply (H j g1) g2 v = 
  let prf1 = indexInjectiveVectDec j n v {prf} 
...
apply {i} {n} (CNOT c t {prf1} {prf2} {prf3} g1) g2 v = 
  let prf4 = indexInjectiveVectDec c n v {prf = prf}
      prf5 = indexInjectiveVectDec t n v {prf = prf}
      prf6 = differentIndexInjectiveVectDec c t n {prf1 = prf3} v {prf2 = prf} {prf3 = prf1} {prf4 = prf2}
...
apply' (H j g1) g2 (x::xs) = 
  let prf1 = indexInjectiveVectDec j n (x::xs) {prf} 
...
apply' {n} (CNOT c t {prf1} {prf2} {prf3} g1) g2 (x::xs) = 
  let prf4 = indexInjectiveVectDec c n (x::xs) {prf = prf}
      prf5 = indexInjectiveVectDec t n (x::xs) {prf = prf}
      prf6 = differentIndexInjectiveVectDec c t n {prf1 = prf3} (x::xs) {prf2 = prf} {prf3 = prf1} {prf4 = prf2}
...
tensor g1 g2 = 
  let p1 = allSmallerRangeVectDec 0 n
      p2 = isInjectiveRangeVectDec n p
      p3 = allSmallerPlus n p (rangeVect 0 n) p1 
      p4 = IsInjectiveSuccDec (allDiffRangeVectDec 0 n) p3
