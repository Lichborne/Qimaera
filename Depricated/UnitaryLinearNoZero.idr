...
data Unitary : Nat -> Type where
  IdGate : {auto prf : LT 0 n} -> Unitary n
  H      : {n : Nat} -> (j : Nat) -> {auto prf : LT j n} -> (1 _ : Unitary n) -> Unitary n
  P      : {n : Nat}  -> (p : Double) -> (j : Nat) -> {auto prf : LT j n} -> (1 _ : Unitary n) -> Unitary n
...
public export partial-- not actually partial, just relevant proof of impossibility does not exist for other cases
apply' : {i : Nat} -> {n : Nat} -> 
        (1 _ : Unitary i) -> (1 _ : Unitary n) -> 
        (1 v : Vect i Nat) -> 
        {auto prf : (IsInjectiveT n v)} -> 
...
adjoint (IdGate {prf = prf}) = (IdGate {prf = prf})
adjoint (H {n} j {prf = prf} g) = (adjoint g) . (H j (IdGate {prf = lemmaCompLT0 n j {prf = prf}}))
adjoint (P {n} p j {prf = prf} g) = (adjoint g) . (P (-p) j (IdGate {prf = lemmaCompLT0 n j {prf = prf}}) )
adjoint (CNOT {n} c t {prf1 = prf1} {prf2 = prf2} {prf3 = prf3} g) = (adjoint g) . (CNOT c t (IdGate {prf = lemmaCompLT0 n c {prf = prf1}}))


---------------------TENSOR PRODUCT----------------------------
|||Make the tensor product of two circuits
public export
tensor : {n : Nat} -> {p : Nat} -> {auto prfN : LT 0 n} ->  {auto prfP : LT 0 p} -> 
        (1 _ : Unitary n) -> ( 1 _ : Unitary p) -> Unitary (n + p)
tensor g1 g2 = 
  ...
  in apply {i = p} {n = n + p} g2 (apply {i=n} {n = n+p} g1 (IdGate {n = n+p} {prf = lemmaTransLTLTE 0 n (n+p) prfN (lemmaLTEAddR n p) }) (rangeVect 0 n) {prf = p4}) (rangeVect n p) {prf = p2}

public export
(#) : {n : Nat} -> {p : Nat} -> {auto prfN : LT 0 n} ->  {auto prfP : LT 0 p} -> (1 _ : Unitary n) -> (1 _ : Unitary p) -> Unitary (n + p)
(#) = tensor
...
public export
tensorn : (n : Nat) -> {auto prf : LT 0 n} -> Unitary 1 -> Unitary n
tensorn 0 _ = absurd prf 
tensorn (S k) {prf} g = case k of 
  0 => g
  S x => g # (tensorn {prf = lemma0LTSucc x } (S x) g)

|||Controls on the n-1 first qubits, target on the last
public export
multipleQubitControlledNOT : (n : Nat) -> {auto prf : LT 0 n} -> Unitary n
multipleQubitControlledNOT 0 = absurd prf
multipleQubitControlledNOT 1 = IdGate {prf = prf}
multipleQubitControlledNOT 2 = CNOT 0 1 (IdGate {prf = prf})
multipleQubitControlledNOT (S (S k)) = controlled (multipleQubitControlledNOT {prf = lemma0LTSucc k } (S k))

||| Tensor product of a Vector of Unitary operators
export
tensorMap : {n : Nat} -> {m : Nat} -> (gates : Vect n (Unitary m)) -> Unitary (n*m)
tensorMap [] = IdGate {prf = absurd}
tensorMap {n = S 0} (gate :: []) =  gate
tensorMap {n = S (S k)} (gate :: gates) {prfM = prf1} {prfN = prf2} = gate # (tensorMap gates)
{-}
rewrite multZeroLeftZero m in rewrite plusZeroRightNeutral m in g
