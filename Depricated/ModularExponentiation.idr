...
inPlaceModularAdder : UnitaryOp t => {i: Nat} -> {n : Nat} 
                                -> (1_ : LVect 2 Qubit) -- these are the controls c1 and c2
                                -> (1_ : LVect 1 Qubit) -- this is the additional ancilla
                                -> (1_ : LVect i Qubit) -- this is a represented in i Qubits
                                -> (1_ : LVect i Qubit) -- this is N represented in i Qubits
                                -> (1_ : LVect (S i) Qubit) -- this is b plus the required additional qubit as the last qubit
                                -> UStateT (t n) (t n) (LPair (LVect (3 + i + i)  Qubit) (LVect ((S i)) Qubit)) -- we collect the 2 controls, ancilla, a, and N in the same output LVect, and b in the other
...
inPlaceModularAdder {n = Z} [c1,c2] [ancilla] as bigNs bs = do 
    aAndN <- reCombine as bigNs
    pure $ ((++) (c1::c2::[ancilla]) aAndN) # (bs)-- vacuous case 1 (due to meta-theoretical (n>i))
inPlaceModularAdder {n = (S k)} [c1,c2] [ancilla] as bigNs bs = do 
    aAndN <- reCombine as bigNs
    pure $ ((++) (c1::c2::[ancilla]) aAndN) # (bs)-- vacuous case 2 (due to meta-theoretical (n>i))
...
allControls: UnitaryOp t => {j:Nat} -> {i: Nat} -> {n : Nat} -> {auto prf: LTE j i} -- This is somewhat superfluous, because we can handle other inputs as well it just won't make sense anyway. if we remove this there is only 1 proof required
                                -> (1_ : LVect 1 Qubit) -- this is the control c
                                -> (1_ : LVect 1 Qubit) -- this is the additional ancilla for the addition operation
                                -> (1_ : LVect j Qubit) -- this is x represented in j qubits
                                -> (1_ : LVect i Qubit) -- this is a represented in i qubits
                                -> (1_ : LVect i Qubit) -- this is N represented in i qubits
                                -> (1_ : LVect (S i) Qubit) -- this is b plus the required additional qubit as the last qubit
                                -> UStateT (t n) (t n) (LPair (LVect (2 + j + i + i)  Qubit) (LVect ((S i)) Qubit))
...
cMultAModN: UnitaryOp t => {i: Nat} -> {n : Nat}
                                -> (1_ : LVect 1 Qubit) -- this is the control c
                                -> (1_ : LVect 1 Qubit) -- this is the additional ancilla for the addition operation
                                -> (1_ : LVect i Qubit) -- this is x represented in i qubits 
                                -> (1_ : LVect i Qubit) -- this is a represented in i qubits
                                -> (1_ : LVect i Qubit) -- this is N represented in i qubits
                                -> (1_ : LVect (S i) Qubit) -- this is b plus the required additional qubit as the last qubit
                                -> UStateT (t n) (t n) (LPair (LVect (2 + i + i + i)  Qubit) (LVect ((S i)) Qubit))
...
inPlaceModularMult: UnitaryOp t => {i: Nat} -> {n : Nat}
                                -> (1_ : LVect 1 Qubit) -- this is the control c
                                -> (1_ : LVect 1 Qubit) -- this is the additional ancilla for the addition operation
                                -> (1_ : LVect i Qubit) -- this is x represented in j qubits
                                -> (1_ : LVect i Qubit) -- this is a represented in i qubits
                                -> (1_ : LVect i Qubit) -- this is a^(-1) mod N represented in i qubits. The classical algorithm for calculating this
                                ...
                                -> (1_ : LVect i Qubit) -- this is N represented in i qubits
                                -> (1_ : LVect (S i) Qubit) -- this should be |0>(S n) because we are actually trying to calculate (ax mod N)
                                -> UStateT (t n) (t n) (LPair (LVect (2 + i + i + i + i)  Qubit) (LVect (S i) Qubit)) -- this time x contains the computationally relevant bit so that is given back separately
...
inPlaceModularMult {i = S h} [c] [ancilla] (xs) (as) (asmodinv) (bigNs) (nils) = do
    ...
    --xs # asmodinvAndN <- splitQubitsAt {prf = lteSiPlusPlusSi {i = h}} (S h) all
    --cAas <- reCombine (c::[ancilla]) as -- 2 + i
    --casmodN <- reCombine cAas asmodinvAndN -- 
    --?all  <- reCombine casmodN finalb
...
inPlaceModularExponentiation: UnitaryOp t => {i: Nat} -> {n : Nat}
                                ...
                                -> (1_ : LVect 1 Qubit) -- this is the additional ancilla for the addition operation
                                -> (1_ : LVect i Qubit) -- this is x represented in j qubits
                                -> (1_ : LVect i Qubit) -- this is a^n represented in i qubits
                                -> (1_ : LVect i Qubit) -- this is a^n^(-1) mod N represented in i qubits. The classical algorithm for calculating this
                                                        -- from a is included above given the theoretical assurances in Beuregard
                                                        -- Note that since the content of the registers has to be prepared elsewhere, this is perfectly fine to assume
                                -> (1_ : LVect i Qubit) -- this is N represented in i qubits
                                -> (1_ : LVect (S i) Qubit) -- this should be |0>(S n) because we are actually trying to calculate (ax mod N)
                                -> UStateT (t n) (t n) (LPair (LVect (3 + i + i + i + i) Qubit) (LVect (i) Qubit))
...
inPlaceModularExponentiation {i = S h} [c] [ancilla] (xs) (ans) (asnmodinv) (bigNs) (nils) = do
    ...
    --LVect (3 + (S (plus (plus (plus h (S h)) (S h)) (S h)))))) Qubit
    --LVect (3 + plus (S h) (plus (plus (S h) (S h)) (S h))) Qubit
    --rewrite plusSuccLeftSucc h  (plus (plus (S h) (S h)) (S h)) in rewrite plusCommutative (h) (plus (plus (S h) (S h)) (S h)) in 
-}
...
inPlaceModularExponentiationNoPrf: UnitaryOp t => {i: Nat} -> {n : Nat}
                                ...
                                -> (1_ : LVect i Qubit) -- this is N represented in i qubits
                                -> (1_ : LVect (S i) Qubit) -- this should be |0>(S n) because we are actually trying to calculate (ax mod N)
                                -> UStateT (t n) (t n) (LPair (LVect (S (S (S (i + (i + i + i))))) Qubit) (LVect (i) Qubit) )
