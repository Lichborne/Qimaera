...
(define csegen-97 (lambda (u--q) (lambda (eta-0) (QStateT-pure u--q eta-0))))
(define ControlC-45LinearC-45LIO-u--bindL_LinearBind_IO (lambda (ext-0 ext-1 ext-2) (let ((act-3 (ext-0 ext-2))) ((ext-1 act-3) ext-2))))
(define csegen-101 (lambda (eta-0) (lambda (eta-1) (lambda (eta-2) (lambda (eta-3) (lambda (eta-4) (ControlC-45LinearC-45LIO-u--bindL_LinearBind_IO eta-2 eta-3 eta-4)))))))
(define csegen-102 (cons (cons 1.0 0.0) '()))
(define csegen-103 (cons csegen-102 '()))
(define csegen-122 (vector csegen-42 (lambda (u--b) (lambda (u--a) (lambda (arg-10411) (lambda (arg-10414) (lambda (world-0) (let ((act-1 (arg-10411 world-0))) ((arg-10414 act-1) world-0))))))) (lambda (u--a) (lambda (arg-10425) (lambda (world-0) (let ((act-1 (arg-10425 world-0))) (act-1 world-0)))))))
(define csegen-125 (cons csegen-122 (lambda (u--a) (lambda (arg-13095) arg-13095))))
(define csegen-128 (lambda (eta-0) (lambda (eta-1) (cons eta-0 eta-1))))
...
(define csegen-133 (cons (lambda (u--x) (PreludeC-45Show-u--show_Show_Nat u--x)) (lambda (u--d) (lambda (u--x) (PreludeC-45Show-u--showPrec_Show_Nat u--d u--x)))))
...
(define csegen-137 (UnitaryLinear-compose UnitaryLinear-HGate UnitaryLinear-SGate))
(define Lemmas-ltz1 1)
(define csegen-157 (cons (vector 0 Lemmas-ltz1) '()))
(define csegen-173 (cons (cons 0.0 0.0) csegen-102))
(define csegen-174 (cons (cons 0.0 0.0) '()))
(define csegen-175 (cons (cons 1.0 0.0) csegen-174))
(define csegen-178 (cons (cons 0.0 0.0) csegen-174))
(define csegen-183 (cons (/ 1.0 (flsqrt 2.0)) 0.0))
(define Lemmas-lemma1LTESucc (lambda (arg-0) (cond ((equal? arg-0 0) 1)(else (let ((e-0 (- arg-0 1))) (Lemmas-lemma1LTESucc e-0))))))
(define csegen-196 (vector 1 0 (Lemmas-lemma1LTESucc 1) (vector 0 )))
(define csegen-230 (vector 1 1 (+ (Lemmas-lemma1LTESucc 1) 1) (vector 0 )))
...
(define Matrix-ket1 (lambda (arg-0) (cond ((equal? arg-0 0) '())(else (let ((e-0 (- arg-0 1))) (cons (cons (cons 1.0 0.0) csegen-102) (Matrix-ket1 e-0)))))))
...
(define Matrix-toTensorBasis (lambda (arg-1) (if (null? arg-1) csegen-103 (let ((e-3 (car arg-1))) (let ((e-4 (cdr arg-1))) (let ((e-7 (car e-3))) (let ((e-8 (cdr e-3))) (let ((e-11 (car e-8))) (let ((e-12 (cdr e-8))) (Matrix-tensorProduct (cons (cons e-7 '()) (cons (cons e-11 '()) '())) (Matrix-toTensorBasis e-4)))))))))))
...
(define Matrix-matrixH (cons (cons csegen-183 (cons csegen-183 '())) (cons (cons csegen-183 (cons (cons (/ -1.0 (flsqrt 2.0)) 0.0) '())) '())))
...
(define Matrix-matrixP (lambda (arg-0) (cons csegen-175 (cons (cons (cons 0.0 0.0) (cons (Complex-cis arg-0) '())) '()))))
...
(define Matrix-simpleTensor (lambda (arg-0 arg-1 arg-2) (cond ((equal? arg-1 0) csegen-103)(else (let ((e-0 (- arg-1 1))) (cond ((equal? arg-2 0) (Matrix-tensorProduct arg-0 (Matrix-simpleTensor arg-0 e-0 (+ e-0 1))))(else (let ((e-1 (- arg-2 1))) (Matrix-tensorProduct (Matrix-matrixId 2) (Matrix-simpleTensor arg-0 e-0 e-1))))))))))
...
(define Matrix-matrixKet0Bra0 (cons csegen-175 (cons csegen-178 '())))
(define Matrix-matrixKet1Bra1 (cons csegen-178 (cons csegen-173 '())))
(define Matrix-matrixX (cons csegen-173 (cons csegen-175 '())))
(define Matrix-tensorCnotAux (lambda (arg-0 arg-1 arg-2) (cond ((equal? arg-0 0) csegen-103)(else (let ((e-0 (- arg-0 1))) (cond ((equal? arg-1 0) (cond ((equal? arg-2 0) (Matrix-tensorProduct (Matrix-matrixId 2) (Matrix-tensorCnotAux e-0 (+ e-0 1) (+ e-0 1))))(else (let ((e-2 (- arg-2 1))) (Matrix-tensorProduct Matrix-matrixKet1Bra1 (Matrix-tensorCnotAux e-0 (+ e-0 1) e-2))))))(else (let ((e-1 (- arg-1 1))) (cond ((equal? arg-2 0) (Matrix-tensorProduct Matrix-matrixX (Matrix-tensorCnotAux e-0 e-1 (+ e-0 1))))(else (let ((e-3 (- arg-2 1))) (Matrix-tensorProduct (Matrix-matrixId 2) (Matrix-tensorCnotAux e-0 e-1 e-3)))))))))))))
...
(define DataC-45Vect-u--toList_Foldable_C-40VectC-32C-36nC-41 (lambda (ext-0) (DataC-45Vect-u--foldr_Foldable_C-40VectC-32C-36nC-41 csegen-128 '() ext-0)))
...
(define UnitaryLinear-controlledH (lambda () (let ((u--h1 (UnitaryLinear-tensor 1 1 (vector 0 ) (UnitaryLinear-compose UnitaryLinear-SAdjGate (UnitaryLinear-compose UnitaryLinear-HGate (UnitaryLinear-compose UnitaryLinear-TGate csegen-137)))))) (let ((u--h2 (UnitaryLinear-tensor 1 1 (vector 0 ) (UnitaryLinear-compose UnitaryLinear-SAdjGate (UnitaryLinear-compose UnitaryLinear-HGate (UnitaryLinear-compose UnitaryLinear-TAdjGate csegen-137)))))) (UnitaryLinear-compose u--h1 (UnitaryLinear-compose UnitaryLinear-CNOTGate u--h2))))))
...
(define QuantumOp-u--applyHQ_QuantumOp_SimulatedOp (lambda (arg-0 arg-1 arg-2 ext-0) (QStateT-C-62C-62C-61 (lambda (eta-0) (QuantumOp-u--applyUST_QuantumOp_SimulatedOp arg-1 1 (let ((e-4 (vector-ref arg-0 3))) ((e-4 arg-1) arg-2)) eta-0)) csegen-97 ext-0)))
...
(define QuantumOp-u--applyPQ_QuantumOp_SimulatedOp (lambda (arg-0 arg-1 arg-2 arg-3 ext-0) (QStateT-C-62C-62C-61 (lambda (eta-0) (QuantumOp-u--applyUST_QuantumOp_SimulatedOp arg-1 1 (let ((e-5 (vector-ref arg-0 4))) (((e-5 arg-1) arg-2) arg-3)) eta-0)) csegen-97 ext-0)))
...
(define Matrix-ket0 (lambda (arg-0) (cond ((equal? arg-0 0) '())(else (let ((e-0 (- arg-0 1))) (cons csegen-175 (Matrix-ket0 e-0)))))))
...
(define QuantumOp-runSimulated (lambda (arg-0 arg-1) (ControlC-45LinearC-45LIO-run csegen-42 csegen-101 (vector 4 1 (arg-1 (vector 0 csegen-103 (vector 0 ) '() 0)) (lambda (_-0) (let ((e-2 (car _-0))) (let ((e-3 (cdr _-0))) (let ((e-4 (vector-ref e-2 0))) (cond ((equal? e-4 0) (cond ((equal? arg-0 0) (ControlC-45LinearC-45LIO-u--pure_Applicative_C-40LC-32C-36ioC-41 csegen-42 '()))(else (let ((e-10 (car e-3))) (let ((e-9 (cdr e-3))) (ControlC-45LinearC-45LIO-u--pure_Applicative_C-40LC-32C-36ioC-41 csegen-42 (cons e-10 e-9)))))))(else (blodwen-error-quit "Nat case not covered")))))))))))
...
(define UnitaryLinear-exportToQiskit (lambda (arg-0 arg-1 arg-2) (let ((u--s (UnitaryLinear-toQiskit arg-0 arg-2))) (lambda (world-0) (let ((act-1 ((SystemC-45FileC-45ReadWrite-writeFile csegen-125 arg-1 u--s) world-0))) (case (vector-ref act-1 0) ((0) (PreludeC-45IO-prim__putStr "Error when writing file\xa;" world-0)) (else (vector 0 ))))))))
...
(define Main-qftAbsTest (lambda () (QuantumOp-runUnitaryOpC-39 4 (lambda (eta-0) (UStateT-C-62C-62C-61 (lambda (eta-1) (QuantumOp-newQubitsUST 0 4 eta-1)) (lambda (u--qs) (lambda (eta-1) (UStateT-C-62C-62C-61 (lambda (eta-2) (QuantumOp-applyUSTSimulatedR 4 4 (QFT-qftUAbs (vector (lambda (u--n) (lambda (u--i) (lambda (arg-9803) (lambda (arg-9806) (lambda (eta-3) (UnitarySimulated-u--applyUnitary_UnitaryOp_Unitary u--n u--i arg-9803 arg-9806 eta-3)))))) (lambda (u--n) (lambda (u--i) (lambda (arg-9819) (lambda (u--ownUnitary) (lambda (eta-3) (UnitarySimulated-u--applyUnitaryOwn_UnitaryOp_Unitary u--n u--i arg-9819 u--ownUnitary eta-3)))))) (lambda (u--n) (lambda (u--i) (lambda (arg-9834) (lambda (eta-3) (UnitarySimulated-u--applyUnitaryAbs_UnitaryOp_Unitary u--n u--i arg-9834 eta-3))))) (lambda (u--n) (lambda (arg-9844) (lambda (eta-3) (UnitarySimulated-u--applyH_UnitaryOp_Unitary u--n arg-9844 eta-3)))) (lambda (u--n) (lambda (arg-9856) (lambda (arg-9859) (lambda (eta-3) (UnitarySimulated-u--applyP_UnitaryOp_Unitary u--n arg-9856 arg-9859 eta-3))))) (lambda (u--n) (lambda (arg-9872) (lambda (arg-9875) (lambda (eta-3) (UnitarySimulated-u--applyCNOT_UnitaryOp_Unitary u--n arg-9872 arg-9875 eta-3))))) (lambda (u--n) (lambda (u--i) (lambda (arg-9890) (lambda (arg-9893) (lambda (eta-3) (UnitarySimulated-u--applyControlledAbs_UnitaryOp_Unitary u--n u--i arg-9890 arg-9893 eta-3)))))) (lambda (u--n) (lambda (u--i) (lambda (u--j) (lambda (arg-9911) (lambda (arg-9914) (lambda (eta-3) (UnitarySimulated-u--applyParallel_UnitaryOp_Unitary u--n u--i u--j arg-9911 arg-9914 eta-3))))))) (lambda (u--i) (lambda (u--n) (lambda (arg-9926) (lambda (eta-3) (UnitarySimulated-u--adjointUST_UnitaryOp_Unitary arg-9926 eta-3))))) (lambda (u--n) (lambda (u--i) (lambda (arg-9934) (lambda (arg-9937) (UnitarySimulated-u--run_UnitaryOp_Unitary u--i arg-9934 arg-9937))))) (lambda (u--n) (lambda (u--i) (lambda (arg-9947) (lambda (arg-9950) (UnitarySimulated-u--exportSelf_UnitaryOp_Unitary u--i arg-9947 arg-9950))))) (lambda (u--n) (lambda (u--i) (lambda (u--j) (lambda (arg-9962) (lambda (arg-9965) (UnitarySimulated-u--runSplit_UnitaryOp_Unitary u--i u--j arg-9962 arg-9965)))))) (lambda (u--n) (lambda (u--i) (lambda (u--j) (lambda (arg-9980) (lambda (eta-3) (UnitarySimulated-u--applyWithSplitLVects_UnitaryOp_Unitary u--n u--i u--j arg-9980 eta-3)))))) (lambda (u--i) (lambda (u--j) (lambda (u--n) (lambda (arg-9995) (lambda (arg-9998) (lambda (eta-3) (UnitarySimulated-u--reCombine_UnitaryOp_Unitary u--i u--j u--n arg-9995 arg-9998 eta-3))))))) (lambda (u--i) (lambda (u--j) (lambda (u--n) (lambda (arg-10016) (lambda (eta-3) (UnitarySimulated-u--reCombineAbs_UnitaryOp_Unitary u--i u--j u--n arg-10016 eta-3)))))) (lambda (u--i) (lambda (u--j) (lambda (u--n) (lambda (arg-10033) (lambda (arg-10036) (lambda (eta-3) (UnitarySimulated-u--applyControlWithSplitLVects_UnitaryOp_Unitary u--i u--j u--n arg-10033 arg-10036 eta-3)))))))) 4 4 u--qs) eta-2)) (lambda (u--out) (lambda (eta-2) (cons eta-2 u--out))) eta-1))) eta-0)))))
(define Main-qftAbsTestIo (lambda () (let ((u--un (Main-qftAbsTest))) (lambda (world-0) (let ((act-1 ((UnitaryLinear-draw 4 u--un) world-0))) (let ((act-2 ((UnitaryLinear-exportToQiskit 4 "qftAbs.py" u--un) world-0))) (vector 0 )))))))
(define Examples-toBellBasis (vector 3 0 1 (Lemmas-lemma1LTESucc 1) 2 (vector 0 1) csegen-196))
...
(define Examples-depthExample2 (vector 1 2 3 (vector 1 1 (+ (Lemmas-lemma1LTESucc 1) 1) (vector 1 0 (Lemmas-lemma1LTESucc 2) csegen-230))))
...
(define Examples-parametrized_example2 (lambda (arg-0 arg-1 arg-2) (UnitaryLinear-compose UnitaryLinear-CNOTGate (UnitaryLinear-compose (cond ((equal? arg-0 1) csegen-196) (else (vector 0 ))) (cond ((equal? arg-1 1) (vector 0 )) (else (vector 2 arg-2 1 2 (vector 0 ))))))))
